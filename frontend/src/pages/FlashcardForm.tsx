      ...
      const fetchCard = async () => {
        try {
          ...
          if (card) {
            ...
            setExample(card.extraInfo?.example || '');
            setTags(card.tags || []);
          }
        } catch (err) {
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold uppercase tracking-wider text-slate-400 px-1">Example / Extra Info (Optional)</label>
            <input
              type="text"
              value={example}
              ...
              placeholder="e.g. Paris is the most populous city of France."
            />
          </div>
